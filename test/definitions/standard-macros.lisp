...
(define-macro-test (defgeneric)
  ...
  '((defgeneric foo ()
      #6=(:method))
    syn:invalid-syntax-error #6# "must be of the for (:method [QUALIFIERS] LAMBDA-LIST [DECLARATION] FORM*)")
  ;; Repeated options
  '((defgeneric foo ()
      (:generic-function-class bar)
      #7=(:generic-function-class bar))
    syn:invalid-syntax-error #7# ":GENERIC-FUNCTION-CLASS option must not be repeated")
  '((defgeneric foo (a)
      (:argument-precedence-order a)
      #8=(:argument-precedence-order a))
    syn:invalid-syntax-error #8# ":ARGUMENT-PRECEDENCE-ORDER option must not be repeated")
  '((defgeneric foo ()
      (:documentation "foo")
      #9=(:documentation "foo"))
