...
(defmacro define-loop-clause (name (&rest arguments) &body body)
  (let ((rule-name (a:symbolicate '#:loop- name '#:-clause)))
    `(defrule (,rule-name :environment (make-instance 'syntax.expression-grammar::expression-environment)) ,arguments
       ,@body)))
...
(defrule (loop-name-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
...
(defrule (loop-variable-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    (or (loop-with-clause)
        (loop-initial-final-clause)
        (loop-for-as-clause)))

(defrule (loop-with-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    (seq (structure 'symbol (symbol-name "WITH")) (<- name ((variable-name! names))))
  (list :with name))

(defrule (loop-for-as-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    ;; TODO destructuring
...
(defrule (loop-main-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
...
(defrule (loop-unconditional-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) (context)
    (or (loop-do-clause)
        (loop-return-clause context)))

(defrule (loop-do-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    (seq (structure 'symbol (symbol-name (or "DO" "DOING")))
         ((compound-form! forms))))

(defrule (loop-return-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) (context)
    (seq (structure 'symbol (symbol-name "RETURN"))
         (loop-form-or-it context)))

(defrule (loop-accumulation-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) (context)
    (or (loop-list-accumulation-clause context)
        (loop-numeric-accumulation-clause context)))

(defrule (loop-list-accumulation-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) (context)
...
(defrule (loop-numeric-accumulation-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) (context)
...
(defrule (loop-conditional-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
...
(defrule (loop-selectable-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    (or (loop-unconditional-clause :conditional)
        (loop-accumulation-clause :conditional)
        (loop-conditional-clause)))

(defrule (loop-termination-test-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
    (seq (structure 'symbol (symbol-name (or "WHILE" "UNTIL" "REPEAT" "ALWAYS" "NEVER" "THEREIS")))
         (<- form ((form! forms))))
  (list :while form))

(defrule (loop-initial-final-clause :environment (make-instance 'syntax.expression-grammar::expression-environment)) ()
