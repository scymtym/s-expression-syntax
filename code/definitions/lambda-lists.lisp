...
(defrule %macro-lambda-list (seen)
    (list* (<- index (:transform (seq) -1))
           (<- env (:transform (seq) nil))
           (<- env-index (:transform (seq) nil))
           (? (seq (<- whole       (whole-section seen))
                   (<- whole-index (:transform (seq) (incf index)))))
           (? (:transform (<- env1 #1=(eg:once (environment-section seen)
                                               :flag env? :name &environment))
                (setf env env1 env-index (incf index))))
           (? (seq (<- required       (destructuring-required-section seen))
                   (<- required-index (:transform (seq) (incf index)))))
           (? (:transform (<- env2 #1#)
                (setf env env2 env-index (incf index))))
           (? (seq (<- optional       (optional-section seen 'nil))
                   (<- optional-index (:transform (seq) (incf index)))))
           (? (:transform (<- env3 #1#)
                (setf env env3 env-index (incf index))))
           (or (list* (? (seq (<- rest       (destructuring-rest-section seen))
                              (<- rest-index (:transform (seq) (incf index)))))
                      (? (:transform (<- env4 #1#)
                           (setf env env4 env-index (incf index))))
                      (? (seq (<- keyword       (keyword-section seen 'nil))
                              (<- keyword-index (:transform (seq) (incf index)))))
                      (? (:transform (<- env5 #1#)
                           (setf env env5 env-index (incf index))))
                      (? (seq (<- aux       (aux-section seen))
                              (<- aux-index (:transform (seq) (incf index)))))
                      (? (:transform (<- env6 #1#)
                           (setf env env6 env-index (incf index))))
                      (lambda-list-junk '"a macro lambda list"))
               (<- cdr ((unique-variable-name! lambda-lists) seen))))
  (list (list whole    whole-index)
        (list env      env-index)
        (list required required-index)
        (list optional optional-index)
        (list rest     rest-index)
        (list keyword  keyword-index)
        (list aux      aux-index)
        (list cdr      10))) ; doesn't matter as long as maximum

(define-syntax macro-lambda-list
    (and (<- seen (:transform :any (make-hash-table :test #'eq)))
         (:compose (%macro-lambda-list seen)
             (list (list whole-section       whole-index)
              (list environment-section environment-index)
              (list required-section    required-index)
              (list optional-section    optional-index)
              (list rest-section        rest-index)
              (list keyword-section     keyword-index)
              (list aux-section         aux-index)
              (list cdr                 cdr-index))))
    (append `(,@environment-section ,@whole-section
              ,@required-section ,@optional-section)
            (if (not (null cdr))
                cdr
                `(,@rest-section ,@keyword-section ,@aux-section)))
  ((whole-section       ? :index whole-index)
   (environment-section ? :index environment-index)
   (required-section    ? :index required-index    :evaluation :compound)
   (optional-section    ? :index optional-index    :evaluation :compound)
   (rest-section        ? :index rest-index        :evaluation :compound)
   (keyword-section     ? :index keyword-index     :evaluation :compound)
   (aux-section         ? :index aux-index         :evaluation :compound)
   (cdr                 ? :index cdr-index)))
...
(define-syntax deftype-lambda-list
    (and (<- seen (:transform :any (make-hash-table :test #'eq)))
         (:compose ((%macro-lambda-list macro-lambda-list) seen)
                   (list (list whole-section       whole-index)
                         (list environment-section environment-index)
                         (list required-section    required-index)
                         (list optional-section    optional-index)
